...
public class ColumnPopupAction extends AbstractAction {
	...
	public ColumnPopupAction(String name, int operation, boolean isSuperColumn,
			TreeNode treeNode) {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		case OPERATION_REMOVE:
			...
			int status = JOptionPane.showConfirmDialog(null, "Delete a " + msg
					+ " " + getName() + "?", "confirm",
	...
	private void insertKeyCell() {
		CellPropertiesDialog cpdlg = new CellPropertiesDialog(
				isSuperColumn ? CellPropertiesDialog.OPERATION_KEY_SUPERCOLUMN_INSERT
    ...
    int ttl = cpdlg.getTtl();
		...
		try {
			...
			if (k == null) {
				Map<String, Key> m = client.getKey(client.getKeyspace(), client
						.getColumnFamily(), superColumn.isEmpty() ? null
						: superColumn, key);
				k = m.get(key);
				if (k == null) {
					k = new Key(key, new HashMap<String, SColumn>(),
		...
		Date d = null;
		try {
			d = client.insertColumn(client.getKeyspace(), client
					.getColumnFamily(), key, s == null ? null : superColumn,
					name, value);
		} catch (Exception e) {
	...
	private void insertCell() {
		...
		CellPropertiesDialog cpdlg = new CellPropertiesDialog(
				isSuperColumn ? CellPropertiesDialog.OPERATION_SUPERCOLUMN_INSERT
		...
		SColumn s = null;
		if (isSuperColumn) {
			s = new SColumn(k, cpdlg.getSuperColumn(),
					new HashMap<String, Cell>());
		}
		...
		try {
			d = client.insertColumn(client.getKeyspace(),
					client.getColumnFamily(), k.getName(),
		...
		try {
			if (treeNode.getUnit() instanceof Key) {
				...
				client.removeSuperColumn(client.getKeyspace(),
						client.getColumnFamily(), k.getName(), s.getName());
				...
				removeNode((DefaultMutableTreeNode) treeNode.getNode()
						.getParent(), treeNode.getNode());
			} else {
				Cell c = (Cell) treeNode.getUnit();
				treeNode.getUnitMap().remove(c.getTreeNode());
				...
				if (parent instanceof Key) {
					...
					client.removeColumn(client.getKeyspace(),
							client.getColumnFamily(), k.getName(), c.getName());
